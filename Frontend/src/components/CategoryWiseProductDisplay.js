...
const CategroyWiseProductDisplay = ({ heading, categoryId }) => {
        ...
        <div className='container mx-auto px-4 my-6 relative'>
            ...
            <div className='grid grid-cols-[repeat(auto-fit,minmax(300px,320px))] justify-between md:gap-6 overflow-x-scroll scrollbar-none transition-all'>
                {loading ? (
                    ...
                    data.map((product) => (
                        <Link to={"/product/" + product?.id} className='w-full min-w-[280px]  md:min-w-[320px] max-w-[280px] md:max-w-[280px]  bg-white rounded-sm shadow ' onClick={scrollTop} key={product?.id}>
                            <div className='bg-slate-200 h-48 p-4 min-w-[280px] md:min-w-[145px] flex justify-center items-center'>
                                <img src={`data:image/jpeg;base64,${product.photoPath}`} className='object-scale-down h-full hover:scale-110 transition-all mix-blend-multiply' alt={product?.productName} />
                            </div>
                            <div className='p-4 grid gap-3'>
